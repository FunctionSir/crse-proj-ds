...
#define INCREM 5
using namespace std;     /*溢出时，顺序表长度的增量*/
int vis[20];//查找时记录序号
char s[20];
int pivot;
...
int ListInsert_sq(Sqlist *L,int i,ElemType e);/*将e插入第i个位置之前,即是实际表中i位置（插入员工信息的函数） */
...
int CreateList_sq(Sqlist *L,int n){/*创建顺序表*/
    ...
    int i;
    for(i=0;i<n;i++){
        printf("输入员工相关数据 位置%d",i+1);
        scanf("%s %s %s %s %s %s %s %s",e.eno,e.name,e.sex,e.date,e.degree,e.job,e.phonenumber,e.address);//输入员工相关信息
...
int PrintList_sq(Sqlist *L){
   ...
   for(i=1;i<=L->length;i++)
        printf("\n在顺序表中的位置:%d\n编号:%s\n姓名:%s\n性别:%s\n出生年月:%s\n学历:%s\n职务:%s\n电话:%s\n地址:%s\n",i,L->slist[i-1].eno,L->slist[i-1].name,L->slist[i-1].sex,L->slist[i-1].date,L->slist[i-1].degree,L->slist[i-1].job,L->slist[i-1].phonenumber,L->slist[i-1].address);
    return OK;
}/*PrintList*/
/*插入员工信息*/
int ListInsert_sq(Sqlist *L,int i,ElemType e)
{
    ...
    if(L->length>=L->listsize)
    {
        L->slist=(ElemType*)realloc(L->slist,
                                    (INIT_SIZE+INCREM)*sizeof(ElemType));//扩大空间
        if(!L->slist)
            return ERROR;
        L->listsize+=INCREM;
    }
    for(k=L->length-1; k>=i-1; k--)      //插入员工数据到顺序表中i-1位置，实际表中i位置
    {
    ...
    case 1:
        {
            ...
            while(i<j&&atoi(L->slist[j].eno)>=pivotkey)--j;
            L->slist[i]=L->slist[j];
            while(i<j&&atoi(L->slist[i].eno)<=pivotkey)++i;
            L->slist[j]=L->slist[i];
        }
        break;
    case 2:
        {
            ...
            while(i<j&&strcmp(L->slist[j].name,pivotkey)>=0)--j;
            L->slist[i]=L->slist[j];
            while(i<j&&strcmp(L->slist[i].name,pivotkey)<=0)++i;
            L->slist[j]=L->slist[i];
        }
        break;
    case 3:
        {
            ...
            while(i<j&&strcmp(L->slist[j].date,pivotkey)>=0)--j;
            L->slist[i]=L->slist[j];
            while(i<j&&strcmp(L->slist[i].date,pivotkey)<=0)++i;
            L->slist[j]=L->slist[i];
        }
        break;
    case 4:
        {
            ...
            while(i<j&&degreerank[L->slist[j].degree]>=pivotkey)--j;
            L->slist[i]=L->slist[j];
            while(i<j&&degreerank[L->slist[i].degree]<=pivotkey)++i;
            L->slist[j]=L->slist[i];
        }
        break;
    }
        ...
        while(1)
        {
            ...
            case 'A':
            {
                ...
                for(int i=1; i<=sl.length; i++)
                {
                    ...
                    {
                        ans++;
                        printf("\n在顺序表中的位置:%d\n编号:%s\n姓名:%s\n性别:%s\n出生年月:%s\n学历:%s\n职务:%s\n电话:%s\n地址:%s\n",i,sl.slist[i-1].eno,sl.slist[i-1].name,sl.slist[i-1].sex,sl.slist[i-1].date,sl.slist[i-1].degree,sl.slist[i-1].job,sl.slist[i-1].phonenumber,sl.slist[i-1].address);
                    }
                }
                if(ans==0)
                {
            ...
            case 'C':
            {
                ...
                printf("\n请输入插入的员工的信息\n");
                ElemType e;
                scanf("%s %s %s %s %s %s %s %s",e.eno,e.name,e.sex,e.date,e.degree,e.job,e.phonenumber,e.address);
