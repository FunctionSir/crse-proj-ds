...
#include <vector>
using namespace std;
int tolnumber, vipnumber, toltime, starthour, startminute, finalhour,
    finalminute, vistime, tal;
typedef struct // 信息
{
    char name[20];
    char cardnumber[6];
    ...
    for (int i = 0; i < 3; i++) {
        while (!emptyf(&que[i])) {
            ...
            if (sum >= s) {
                ...
                pop(&que[i]);
                if (!emptyf(&que[i])) {
                    toltime += s - que[i].cs[que[i].front].starttimehour * 60 -
                               que[i].cs[que[i].front].starttimeminute;
...
void over() {
    ...
    for (int i = 0; i < 3; i++) {
        t = 0;
        while (!emptyf(&que[i])) {
            int s = que[i].cs[que[i].front].starttimehour * 60 +
                    que[i].cs[que[i].front].starttimeminute +
                    que[i].cs[que[i].front].durtime;
            t = s;
            toltime += que[i].cs[que[i].front].durtime;
            pop(&que[i]);
            if (!emptyf(&que[i])) {
                toltime += t - que[i].cs[que[i].front].starttimehour * 60 -
                           que[i].cs[que[i].front].starttimeminute;
                que[i].cs[que[i].front].starttimehour = s / 60;
                que[i].cs[que[i].front].starttimeminute = s % 60;
            }
...
void sit() {
    ...
    while (1) {
        ...
        case 1: {
            ...
            if (t.vip > 0) {
                for (int i = 1; i <= tal; i++) {
                    if (strcmp(vipfile[i].name, t.name) == 0 &&
                        strcmp(t.cardnumber, vipfile[i].cardnumber) == 0) {
                        u = 1;
                    }
                }
                if (u == 0) {
            ...
            update(t.starttimehour,
                   t.starttimeminute); // 跟新三个窗口的队伍的进度
            if (t.vip > 0) {
                vipnumber++;
                s1.push_back(t);
            }
        ...
        case 5: {
            for (int i = 1; i <= tal; i++) {
                printf("\n名字:%s\n卡号:%s\n的存款:%d\n", vipfile[i].name,
                       vipfile[i].cardnumber, vipfile[i].deposit);
                if (vipfile[i].vip == 1) {
                    printf("优先级:I级\n");
